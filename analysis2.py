...
with col1:
    ...
    if fl1 is not None:
        filename = fl1.name
        st.write(filename)
        df1 = pl.read_csv(filename)
    else:
        os.chdir(r"/Users/evgeniyakiseleva/ds_projects/bolid")
        df1 = pl.read_csv("checks.csv")
with col2:
    fl2 = st.file_uploader(":file_folder: ЗАГРУЗИТЕ ФАЙЛ ITEMS",type=(["csv"]))
    if fl2 is not None:
        filename = fl2.name
        st.write(filename)
        df2 = pl.read_csv(filename)
    else:
        os.chdir(r"/Users/evgeniyakiseleva/ds_projects/bolid")
        df2 = pl.read_csv("items.csv")
    
df1 = df1.select([pl.col(name).alias(name.lower()) for name in df1.columns])

df2 = df2.select([pl.col(name).alias(name.lower()) for name in df2.columns])

df1=df1.rename({'дата/время':'date_time','магазин':'seller','имя кассы':'cashier',\
                   ...
                   'электронными':'e_pay','итого':'total_pay','статус':'status','город':'city'})
df2=df2.rename({'фп':'ff','номер документа':'doc_number','дата/время':'date_time',\
                   ...
                   'количество единиц измерения в чеке':'changes_in_check',\
                    ...
                    'название игры':'game_name','группа игры':'group_game'})
df1=df1.set_sorted(df1.select(pl.col('seller','cashier','calculation','city','status').str.to_lowercase()))
df2=df2.set_sorted(df2.select(pl.col('service','game_name','group_game').str.to_lowercase()))
df2.drop_in_place('service')
df=df2.join(df1, on=['ff','date_time','doc_number','shift_number'], how="left")
df.drop_in_place('final_check')
df.drop_in_place('cash_check')
df.drop_in_place('e_pay_check')
df.drop_in_place('ff')
df.drop_in_place('doc_number')
df.drop_in_place('shift_number')
df.drop_in_place('check_number')
df.drop_in_place('position_in_check')
df.drop_in_place('cashier')
df.drop_in_place('status')
...
with cl2:
    with st.expander("Продажи по точкам продаж"):
        ...
        csv = seller_sales.to_csv(index = False)
